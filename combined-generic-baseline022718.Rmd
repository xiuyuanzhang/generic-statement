...
ggplot(data=full_data_by_type, aes(x=type, y=mean, color=condition)) + 
  geom_pointrange(aes(ymin = mean-std_err, ymax = mean+std_err), shape = 20, size = 0.4,position=position_dodge(.5)) +
  coord_cartesian(ylim=c(0,100)) +
  ylab("Percent(%)") +
  xlab("Prevalence Estimation Levels") +
  ggtitle("Feature Prevalence Estimation(Explicit)")
